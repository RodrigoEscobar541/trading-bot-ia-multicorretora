...
4. **Nunca vender no prejuízo POR DECISÃO DA IA — avaliado POR POSIÇÃO.** Cada
   ...
   **Primeira exceção — STOP-LOSS (V6.6, ver §11).** A venda no prejuízo existe em um
...
## 5.1 Regra de Stop-Loss (V6.6) — a única venda no prejuízo
...
- Permite editar por ativo: liga/desliga, modo simulação, intervalos, variação mínima, divergência máxima, reset, taxas, orçamento, mínimos, prompt e contexto. Por plataforma: chaves de API (mascaradas), cadeia de modelos da IA e template.
